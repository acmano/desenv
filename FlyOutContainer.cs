...
//  private const int                  BezelArea = 30; //dip
    ...
    private readonly ITimeInterpolator _interpolator = new SmoothInterpolator ();
    private VelocityTracker            _velocityTracker;
    private bool                       _stateBeforeTracking;
    private bool                       _isTracking;
    private bool                       _preTracking;
    private int                        _startX = -1;
    private int                        _startY = -1;
    private int                        _touchSlop;
    private int                        _pagingTouchSlop;
    private int                        _minFlingVelocity;
    private int                        _maxFlingVelocity;
    private GradientDrawable           _shadowDrawable;
    ...
    public FlyOutContainer (Context context, IAttributeSet attrs) : base (context, attrs)
    {
      Initialize ();
    }

    public FlyOutContainer (Context context, IAttributeSet attrs, int defStyle) : base (context, attrs, defStyle)
    {
      Initialize ();
    }
    ...
    void Initialize ()
    {
      ...
      _shadowDrawable = new GradientDrawable (GradientDrawable.Orientation.RightLeft, shadowColors);
    ...
    View ContentView
    {
      get
      {
        return FindViewById (Resource.Id.consulta00);
      }
    }
    ...
    View MenuView
    {
      get
      {
        return FindViewById (Resource.Id.menulateral);
      }
    }
    ...
    private void SetOpened (bool opened, bool animated = true)
    {
      _opened = opened;
      if (!animated)
      {
    ...
    void SetNewOffset (int newOffset)
    {
      ...
      {
        if ( !_opened && _contentOffsetX == MaxOffset )
        {
          _opened = true;
        }
      }
    ...
    public override bool OnTouchEvent (MotionEvent e)
    {
      ...
      if ( e.Action == MotionEventActions.Move )
      {
        ...
        SetNewOffset ( _stateBeforeTracking ? MaxOffset - Math.Min ( MaxOffset, traveledDistance ) : Math.Max ( 0, traveledDistance ) );
      }
      else
      {
        if (e.Action == MotionEventActions.Up && _stateBeforeTracking == _opened) 
        {
          ...
          {
            if (!_opened && _contentOffsetX > MaxOffset / 2)
            {
              SetOpened (true);
            }
            else
    ...
    bool CaptureMovementCheck (MotionEvent ev)
    {
      if (ev.Action == MotionEventActions.Down)
      {
        ...
        if (!_opened && (_startX > Context.ToPixels(30)))
        {
          return false;
        }
    ...
    {
      return (_stateBeforeTracking ? e.GetX () <= _startX : e.GetX () >= _startX) && Math.Abs (e.GetY () - _startY) < _touchSlop;
    }

    protected override void DispatchDraw (Canvas canvas)
    {
      base.DispatchDraw (canvas);
      if (_opened || _isTracking || _animator != null)
      {
